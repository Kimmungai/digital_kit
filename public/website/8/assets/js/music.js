...
Carousel.prototype.go_to = function( index, currentIndex )
{
	if (currentIndex != index) { //Avoid to start over the audio by clicking on the current music and mute the audio
		...
		/* Make visible or unseen the speakers*/
		this.$.speakers[currentIndex].classList.add('unseen');
		this.$.speakers[index].classList.remove('unseen');
...
Carousel.prototype.changeMusic = function()
{
	...
	if (play == true) {
		this.$.play[0].classList.add('unseen');
		this.$.music[0].play();
	}
	else {
		this.$.play[0].classList.remove('unseen');
		this.$.music[0].pause();
	}
};
